...
from utils.indoor6 import Indoor6
def adjust_array_shape(arr, target_shape):
    a, b = target_shape
    current_shape = arr.shape
    
    # Check if the array needs to be adjusted
    if current_shape == (a, b):
        return arr

    # Initialize a new array with the target shape
    new_arr = np.zeros((a, b), dtype=arr.dtype)

    # Determine the minimum shape to copy from the original array
    min_a = min(a, current_shape[0])
    min_b = min(b, current_shape[1])

    # Copy the relevant part of the original array
    new_arr[:min_a, :min_b] = arr[:min_a, :min_b]

    # If padding is needed, pad with the last value
    if current_shape[0] < a:
        for i in range(current_shape[0], a):
            new_arr[i, :min_b] = arr[current_shape[0] - 1, :min_b]
    if current_shape[1] < b:
        for i in range(current_shape[1], b):
            new_arr[:min_a, i] = arr[:min_a, current_shape[1] - 1]

    # If both dimensions need padding
    if current_shape[0] < a and current_shape[1] < b:
        new_arr[current_shape[0]:, current_shape[1]:] = arr[current_shape[0] - 1, current_shape[1] - 1]
...
if __name__ == '__main__':
    ...
    monodepth_folder = os.path.join(opt.dataset_folder, opt.scene_id, 'denseMaps')

    from utils.read_write_models import *
    cameras, images, points = read_model(os.path.join(opt.dataset_folder, '%s/colmap/dense/sparse' % opt.scene_id), ext='.bin')
    ...
    for idx, batch in enumerate(tqdm(dataloader)):        
        ...
        if dmonodense_scaled is not None:
            ...
            H2=np.shape(dmonodense_scaled)[0]
            W2=np.shape(dmonodense_scaled)[1]

            Cplm = batch['landmark3d'][0].cpu().numpy()            
            pixlm = K @ Cplm
            px = pixlm[0] / pixlm[2]
            py = pixlm[1] / pixlm[2]
            infront_infrustum = (Cplm[2] > 0.3) * (Cplm[2] < 15.0) * (px >= 0) * (px < W) * (py >=0) * (py < H) * (px < W2) * (py < H2)
    ...
    for lm_idx in tqdm(range(visibility.shape[0])):
        ...
        pose_paths = [os.path.join(root_folder, scene_id, 'extractedData', ifile.replace('color.jpg', 'pose.txt')) for ifile in images_observe_lm]
        depth_paths = [os.path.join(root_folder, scene_id, 'DenseMaps', ifile.replace('.jpg', '.scaled_depth.npy')) for ifile in images_observe_lm]
        intrinsic_paths = [os.path.join(root_folder, scene_id, 'extractedData', ifile.replace('color.jpg', 'intrinsics.txt')) for ifile in images_observe_lm]
        ...
        for i in range(Ts.shape[0]):
            if os.path.exists(depth_paths[i]):
                ...
                if proj_x >= patch_size and proj_x < W-patch_size and proj_y >= patch_size and proj_y < H-patch_size:
                    ...
                    adjusted_depth=adjust_array_shape(np.load(depth_paths[i]),[H,W])
                    d = adjusted_depth[patch_y0:patch_y1, patch_x0:patch_x1].reshape((1, patch_size * 2, patch_size * 2))
