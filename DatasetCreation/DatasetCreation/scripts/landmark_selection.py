...
import os
import pickle
from utils.read_write_models import qvec2rotmat, read_model
    ...
    for i, pid in enumerate(selected_landmarks['id']):
        for imgid in points3D[pid].image_ids:
            if images[imgid].name in indoor6_imagename_to_index:
                visibility_mask[i, int(indoor6_imagename_to_index[images[imgid].name])] = 1
...
if __name__ == '__main__':
    ...
    path = os.path.join(opt.dataset_folder, '%s/colmap/dense/sparse/' % scene)
    
    cameras, images, points3D = read_model(path, ext='.bin')
    saveLandmarksPath=os.path.join(opt.dataset_folder,opt.scene_id,"landmarks/")
    if(not os.path.exists(saveLandmarksPath)):
        os.makedirs(saveLandmarksPath)
